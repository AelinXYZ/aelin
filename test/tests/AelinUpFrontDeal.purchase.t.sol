...
contract AelinUpFrontDealPurchaseTest is Test, AelinTestUtils {
    ...
    function testFuzz_AcceptDeal_RevertWhen_AmountOverAllocation(uint256 _purchaseAmount) public {
        AelinNftGating.NftPurchaseList[] memory nftPurchaseList;
        vm.startPrank(user1);
    ...
    function testFuzz_AcceptDeal_RevertWhen_AmountTooSmall(uint256 _purchaseAmount) public {
        AelinNftGating.NftPurchaseList[] memory nftPurchaseList;
        vm.startPrank(user1);
        deal(address(purchaseToken), user1, type(uint256).max);
        purchaseToken.approve(address(dealAddressAllowList), type(uint256).max);
    ...
    function test_AcceptDeal_RevertWhen_ERC721WalletAlreadyUsed() public {
        vm.startPrank(user1);
        ...
        MockERC721(collection721_2).mint(user1, 2);
    ...
    function test_AcceptDeal_ERC1155() public {
        ...
        (bool walletClaimed, bool NftIdUsed, bool hasNftList) = AelinUpFrontDeal(dealAddressNftGating1155)
            .getNftGatingDetails(address(collection1155_1), user1, 1);
        assertFalse(walletClaimed);
        assertTrue(NftIdUsed);
        assertTrue(hasNftList);
