...
const sourceWallets = require('./third-aelin-distribution-source.json');
...
const SynthetixStateContractAddressL1 =
...
const providerL2 = new ethers.providers.JsonRpcProvider('https://optimism-mainnet.infura.io/v3/018f3cd9ce7a4195907762b9da21e859');
...
const run = async() => {
	...
	const filteredSourceWallets = Object.keys(sourceWallets).filter(key => !missedAddresses[key] && !firstDistributionList[key]);
	console.log(`Number of wallets which were not already part of the distribution : ${filteredSourceWallets.length}`);
	const eligibleWalletsL1 = (await runAllQueries(filteredSourceWallets, 'L1')).filter(({initialDebtOwnership, collateral}) => Number(initialDebtOwnership) > 0 && Number(collateral)  > 1);
	const eligibleWalletsL2 = (await runAllQueries(filteredSourceWallets, 'L2')).filter(({initialDebtOwnership, collateral}) => Number(initialDebtOwnership) > 0 && Number(collateral)  > 1);
	...
	for (let i = 0; i < eligibleWalletsL1.length; i++) {
		...
		if (output[ethers.utils.getAddress(holder.address)]) {
			...
			output[ethers.utils.getAddress(holder.address)].collateral += Number(
				holder.collateral
			);
		} else {
