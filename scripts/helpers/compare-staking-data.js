...
const SynthetixContractAddress = '0xc011a73ee8576fb46f5e1c5751ca3b9fe0af2a6f';
...
const synthetixContract = new ethers.Contract(SynthetixContractAddress, SynthetixABI, provider);
...
const loadLastDebtLedgerEntry = async (address) => {
	...
	return {
		initialDebtOwnership: issuanceData.initialDebtOwnership,
		debtEntryIndex: issuanceData.debtEntryIndex,
...
const runAllQueries = async (source) => {
	...
	while (batches.length) {
		...
		const result = await Promise.all(
			batch.map(({ HolderAddress }) => loadLastDebtLedgerEntry(HolderAddress))
		);
		results.push(result);
	}
...
fs.createReadStream('./etherscan-source.csv')
	...
	.on('end', async () => {
		...
		const arr = results
			.map(({ initialDebtOwnership, debtEntryIndex, collateral }, i) => {
				return {
		...
		fs.writeFileSync(`./missing-wallets.json`, JSON.stringify(arr), function (err) {
