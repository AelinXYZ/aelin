...
const SynthetixStateContractAddressL2 =
  "0x9770239D49Db97E77fc5Adcb5413654C9e45A510";
...
const run = async () => {
  console.log(
    `Number of wallets from initial list: ${Object.keys(sourceWallets).length}`
  );
  ...
  const eligibleWalletsL1 = (
    await runAllQueries(filteredSourceWallets, "L1")
  ).filter(
    ({ initialDebtOwnership, collateral }) =>
      Number(initialDebtOwnership) > 0 && Number(collateral) > 1
  );
  const eligibleWalletsL2 = (
    await runAllQueries(filteredSourceWallets, "L2")
  ).filter(
    ({ initialDebtOwnership, collateral }) =>
      Number(initialDebtOwnership) > 0 && Number(collateral) > 1
  );
