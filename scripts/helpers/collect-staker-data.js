...
const DebtCacheABI = ['function currentDebt() view returns (uint256 debt, bool anyRateIsInvalid)'];

const SynthetixStateABI = ['function lastDebtLedgerEntry() view returns (uint256)'];
...
const synthetixSnxL1 = 'https://api.thegraph.com/subgraphs/name/synthetixio-team/synthetix';
const synthetixSnxL2 = 'https://api.thegraph.com/subgraphs/name/synthetixio-team/optimism-main';
...
const loadLastDebtLedgerEntry = async (provider, snapshot) => {
	const contract = new ethers.Contract(SynthetixStateContractAddress, SynthetixStateABI, provider);
...
const loadTotalDebt = async (provider, snapshot) => {
	const contract = new ethers.Contract(DebtCacheContractAddress, DebtCacheABI, provider);
...
const createStakersQuery = (blockNumber, minTimestamp) => {
	const minTimestampWhere = minTimestamp != null ? `, timestamp_lt: $minTimestamp` : '';
	return gql`
...
async function getSNXHolders(
	...
	const parsedData = stakedResponse.snxholders.map((holder) => {
		holder.initialDebtOwnership = ethers.utils.formatEther(holder.initialDebtOwnership);
...
async function main() {
	...
	const parsedMissingGraphData = missingGraphData.map((holder) => {
		holder.initialDebtOwnership = ethers.utils.formatEther(
			ethers.utils.parseEther(new ethers.BigNumber.from(holder.initialDebtOwnership).toString())
		);
		holder.debtEntryAtIndex = ethers.utils.formatEther(
			ethers.utils.parseEther(new ethers.BigNumber.from(holder.debtEntryIndex).toString())
		);
		return holder;
	});
	...
	const lastDebtLedgerEntryL1 = await loadLastDebtLedgerEntry(l1Provider, l1BlockNumber);
	...
	if (l1Results.length > 0 && l2Results.length > 0) {
		for (let i = 0; i < l1Results.length; i++) {
			...
			if (readableOutput[getAddress(holder.id)]) {
				console.log('should never have a duplicate in L1 results but we do with:', holder.id);
				readableOutput[getAddress(holder.id)].score += Number(vote);
				readableOutput[getAddress(holder.id)].collateral += Number(holder.collateral);
			} else {
	...
	for (let i = 0; i < accountsValues.length; i++) {
		let newValue =
			Math.round(POST_FLOOR_DISTRIBUTION_AMOUNT * (accountsValues[i].score / Number(totalScore))) +
			FLOOR_AMOUNT;
