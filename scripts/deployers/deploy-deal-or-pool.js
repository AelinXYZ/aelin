...
async function main() {
	...
	await create2Factory
		.connect(deployer)
		.deploy(
			process.env.TYPE === 'DEAL' ? AelinDealArtifact.bytecode : AelinPoolArtifact.bytecode,
			salt
		);

	const dealDeployedLogs = await create2Factory.queryFilter(create2Factory.filters.Deployed());
